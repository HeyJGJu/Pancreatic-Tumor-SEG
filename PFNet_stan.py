...
#from backbone.unet.unet_part import *
from backbone.unet.stan_part import *
...
# ########################## NETWORK ##############################
###################################################################
class PFNet_unet(nn.Module):
    def __init__(self,n_channels, n_classes, bilinear=True, backbone_path=None):
        super(PFNet_unet, self).__init__()
        # params
        ...
        self.inc_3_3 = DoubleConv(n_channels, 64)
        self.inc_1_3 = DoubleConv_1_3(n_channels, 64)
        self.inc_5_3 = DoubleConv_5_3(n_channels, 64)

        #
        self.down1 = Down(64, 128)
        self.down1_com = Down_com(128, 128)

        self.down2 = Down(128, 256)
        self.down2_com = Down_com(256, 256)

        self.down3 = Down(256, 512)
        self.down3_com = Down_com(512, 512)

        self.down4 = Down(512, 512)
        self.down4_com = Down_com(1024, 512)

        self.up1 = Up(2048, 256, bilinear)
        ...
        self.cr3 = nn.Sequential(nn.Conv2d(256, 256, 3, 1, 1), nn.BatchNorm2d(256), nn.ReLU())
    ...
    def forward(self, x):
        ...
        x0 = self.inc_3_3(x)
        x0_1_3 = self.inc_1_3(x)
        x0_5_3 = self.inc_5_3(x)
        x0_com = torch.cat([x0_1_3, x0_5_3], dim=1)

        x1 = self.down1(x0)
        x1_com = self.down1_com(x0_com)

        x2 = self.down2(x1)
        x2_com = self.down2_com(x1_com)

        x3 = self.down3(x2)
        x3_com = self.down3_com(x2_com)

        x4 = self.down4(x3)
        x4_com = self.down4_com(x3_com)

        x4_all = torch.cat([x4, x4_com], dim=1)

        layer3 = self.up1(x4_all, x3)

        layer2 = self.up2(layer3, x2)
        layer1 = self.up3(layer2, x1)
        layer0 = self.up4(layer1, x0)
