...
#import backbone.unet.unet_down as UNet_down
import backbone.unet.unet_up as unet_up
...
class PFNet_unet(nn.Module):
    def __init__(self,n_channels, n_classes, bilinear=True, backbone_path=None):
        ...
        # channel reduction
        self.cr4 = nn.Sequential(nn.Conv2d(1024, 512, 3, 1, 1), nn.BatchNorm2d(512), nn.ReLU())
        self.cr3 = nn.Sequential(nn.Conv2d(512, 256, 3, 1, 1), nn.BatchNorm2d(256), nn.ReLU())
        self.cr2 = nn.Sequential(nn.Conv2d(256, 128, 3, 1, 1), nn.BatchNorm2d(128), nn.ReLU())
        self.cr1 = nn.Sequential(nn.Conv2d(128, 64, 3, 1, 1), nn.BatchNorm2d(64), nn.ReLU())
    ...
    def forward(self, x):
        #backbone
        layer0 = self.inc(x)
        layer1 = self.down1(layer0)
        layer2 = self.down2(layer1)
        layer3 = self.down3(layer2)
        layer4 = self.down4(layer3)
